                ...
                <tbody>
                    <?php if (!empty($categories)): ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="status-<?= $category['status'] ?>"><?= ucfirst($category['status']) ?></span>
                            </td>
                            <td><?= date('M j, Y', strtotime($category['created_at'])) ?></td>
                            <td>
                                <a href="edit-category.php?id=<?= $category['id'] ?>" style="color: #007bff; text-decoration: none;">Edit</a> |
                                <a href="delete-category.php?id=<?= $category['id'] ?>" style="color: #dc3545; text-decoration: none;" onclick="return confirm('Delete this category?')">Delete</a>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="7" style="text-align: center; color: #6c757d;">No categories found</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
